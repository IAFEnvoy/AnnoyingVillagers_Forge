...
public class Null1Entity extends Monster {
    public Null1Entity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.NULL_1.get(), world);
    }

    public Null1Entity(EntityType<Null1Entity> type, Level world) {
        super(type, world);
        this.maxUpStep = 60.0f;
        this.xpReward = 80;
        this.setNoAi(false);
        this.setPersistenceRequired();
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(AnnoyingVillagersbychentuModItems.DIAMOND_S.get()));
    ...
    public static AttributeSupplier.Builder createAttributes() {
        ...
        return builder.add(Attributes.MOVEMENT_SPEED, 2.0d).add(Attributes.MAX_HEALTH, 40.0d).add(Attributes.ARMOR, 40.0d).add(Attributes.ATTACK_DAMAGE, 8.0d).add(Attributes.FOLLOW_RANGE, 128.0d).add(Attributes.FLYING_SPEED, 2.0d);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(23, new MeleeAttackGoal(this, 2.0d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.Null1Entity.1
            @Override
            protected double getAttackReachSqr(LivingEntity entity) {
                return (this.mob.getBbWidth() * this.mob.getBbWidth()) + entity.getBbWidth();
            }
        });
        this.goalSelector.addGoal(24, new RandomStrollGoal(this, 0.4d, 20) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.Null1Entity.2
            @Override
            protected Vec3 getPosition() {
                Random random = Null1Entity.this.getRandom();
                double dir_x = Null1Entity.this.getX() + (((random.nextFloat() * 2.0f) - 1.0f) * 16.0f);
                double dir_y = Null1Entity.this.getY() + (((random.nextFloat() * 2.0f) - 1.0f) * 16.0f);
                double dir_z = Null1Entity.this.getZ() + (((random.nextFloat() * 2.0f) - 1.0f) * 16.0f);
                return new Vec3(dir_x, dir_y, dir_z);
            }
        });
        this.goalSelector.addGoal(25, new LookAtPlayerGoal(this, LingZhiEntity.class, 6.0f));
        this.goalSelector.addGoal(26, new FloatGoal(this));
        this.targetSelector.addGoal(27, new HurtByTargetGoal(this));
        this.goalSelector.addGoal(28, new Goal() { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.Null1Entity.3
            {
            ...
            public boolean canUse() {
                return Null1Entity.this.getTarget() != null && !Null1Entity.this.getMoveControl().hasWanted();
            }

            @Override
            public boolean canContinueToUse() {
                return Null1Entity.this.getMoveControl().hasWanted() && Null1Entity.this.getTarget() != null && Null1Entity.this.getTarget().isAlive();
            }

            @Override
            public void start() {
                LivingEntity livingentity = Null1Entity.this.getTarget();
                Vec3 vec3d = livingentity.getEyePosition(1.0f);
                Null1Entity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 2.0d);
            }

            @Override
            public void tick() {
                Entity getTarget = Null1Entity.this.getTarget();
                if (Null1Entity.this.getBoundingBox().intersects(getTarget.getBoundingBox())) {
                    Null1Entity.this.doHurtTarget(getTarget);
                    return;
                }
                double d0 = Null1Entity.this.distanceToSqr(getTarget);
                if (d0 < 16.0d) {

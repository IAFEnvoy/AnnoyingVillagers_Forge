...
public class LianyingfenshengEntity extends Monster {
    public LianyingfenshengEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.LIANYINGFENSHENG.get(), world);
    }

    public LianyingfenshengEntity(EntityType<LianyingfenshengEntity> type, Level world) {
        super(type, world);
        this.maxUpStep = 2.6f;
        this.xpReward = 40;
        this.setNoAi(false);
        ...
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(AnnoyingVillagersbychentuModItems.LIANFU.get()));
    }

    @SubscribeEvent
    public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
        event.getSpawns().getSpawner(MobCategory.CREATURE).add(new MobSpawnSettings.SpawnerData(AnnoyingVillagersbychentuModEntities.LIANYINGFENSHENG.get(), 2, 1, 1));
    }

    public static void init() {
        SpawnPlacements.register((EntityType) AnnoyingVillagersbychentuModEntities.LIANYINGFENSHENG.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
            return world.getBlockState(pos.below()).getMaterial() == Material.GRASS && world.getRawBrightness(pos, 0) > 8;
        });
        DungeonHooks.addDungeonMob(AnnoyingVillagersbychentuModEntities.LIANYINGFENSHENG.get(), 180);
    }

    public static AttributeSupplier.Builder createAttributes() {
        AttributeSupplier.Builder builder = Mob.createMobAttributes();
        return builder.add(Attributes.MOVEMENT_SPEED, 0.35d).add(Attributes.MAX_HEALTH, 40.0d).add(Attributes.ARMOR, 20.0d).add(Attributes.ATTACK_DAMAGE, 5.0d).add(Attributes.FOLLOW_RANGE, 128.0d);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(25, new MeleeAttackGoal(this, 1.5d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.LianyingfenshengEntity.1
            @Override
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
        ...
        LianyingfenshengShiTiChuShiShengChengShiProcedure.execute(world, this.getX(), this.getY(), this.getZ(), this);
        return retval;
    }

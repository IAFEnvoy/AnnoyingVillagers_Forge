...
public class LanmeikuileiEntity extends IronGolem {
    public LanmeikuileiEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.LANMEIKUILEI.get(), world);
    }

    public LanmeikuileiEntity(EntityType<LanmeikuileiEntity> type, Level world) {
        super(type, world);
        this.maxUpStep = 0.6f;
        this.xpReward = 10;
        this.setNoAi(false);
    ...
    public static AttributeSupplier.Builder createAttributes() {
        ...
        return builder.add(Attributes.MOVEMENT_SPEED, 0.5d).add(Attributes.MAX_HEALTH, 300.0d).add(Attributes.ARMOR, 25.0d).add(Attributes.ATTACK_DAMAGE, 14.0d).add(Attributes.FOLLOW_RANGE, 128.0d).add(Attributes.KNOCKBACK_RESISTANCE, 10.0d).add(Attributes.ATTACK_KNOCKBACK, 30.0d);
    }
    ...
    protected void registerGoals() {
        super.registerGoals();
        this.goalSelector.addGoal(1, new MeleeAttackGoal(this, 1.2d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.LanmeikuileiEntity.1
            @Override
        ...
        this.goalSelector.addGoal(16, new MeleeAttackGoal(this, 1.5d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.LanmeikuileiEntity.2
            @Override
            protected double getAttackReachSqr(LivingEntity entity) {
                return (this.mob.getBbWidth() * this.mob.getBbWidth()) + entity.getBbWidth();
            }

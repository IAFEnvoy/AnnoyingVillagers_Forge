...
import pugilist_steve.mod.annoying_villagersbychentu.procedures.EnchanterDangShiTiGengXinKeShiProcedure;
import pugilist_steve.mod.annoying_villagersbychentu.procedures.EnchanterDangShiTiShouShangShiProcedure;
import pugilist_steve.mod.annoying_villagersbychentu.procedures.EnchanterDangShiTiSiWangShiProcedure;
import pugilist_steve.mod.annoying_villagersbychentu.procedures.EnchanterShiTiChuShiShengChengShiProcedure;
...
public class EnchanterEntity extends Monster {
    public EnchanterEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.ENCHANTER.get(), world);
    }
    ...
    public EnchanterEntity(EntityType<EnchanterEntity> type, Level world) {
        ...
        this.xpReward = 30;
        this.setNoAi(false);
        this.setCustomName(new TextComponent("§5EC"));
        this.setCustomNameVisible(true);
        this.setPersistenceRequired();
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(AnnoyingVillagersbychentuModItems.LU_BAO_SHI_CHAN.get()));
        this.setItemSlot(EquipmentSlot.OFFHAND, new ItemStack(AnnoyingVillagersbychentuModItems.DIAMOND_SHIELD.get()));
    ...
    public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
        event.getSpawns().getSpawner(MobCategory.CREATURE).add(new MobSpawnSettings.SpawnerData(AnnoyingVillagersbychentuModEntities.ENCHANTER.get(), 1, 1, 1));
    }

    public static void init() {
        SpawnPlacements.register((EntityType) AnnoyingVillagersbychentuModEntities.ENCHANTER.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new MeleeAttackGoal(this, 1.5d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.EnchanterEntity.1
            @Override
    ...
    public boolean hurt(DamageSource source, float amount) {
        EnchanterDangShiTiShouShangShiProcedure.execute(this);
    ...
    public void die(DamageSource source) {
        super.die(source);
        EnchanterDangShiTiSiWangShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
        SpawnGroupData retval = super.finalizeSpawn(world, difficulty, reason, livingdata, tag);
        EnchanterShiTiChuShiShengChengShiProcedure.execute(world, this);
        return retval;
    }
    ...
    public void baseTick() {
        super.baseTick();
        EnchanterDangShiTiGengXinKeShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }
}

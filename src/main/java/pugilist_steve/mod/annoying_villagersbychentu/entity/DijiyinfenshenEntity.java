...
public class DijiyinfenshenEntity extends Monster {
    public DijiyinfenshenEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.DIJIYINFENSHEN.get(), world);
    }

    public DijiyinfenshenEntity(EntityType<DijiyinfenshenEntity> type, Level world) {
        super(type, world);
        this.maxUpStep = 2.0f;
        this.xpReward = 0;
        this.setNoAi(false);
        this.setCustomName(new TextComponent("§5herobrine§f低级影分身"));
        this.setCustomNameVisible(true);
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(AnnoyingVillagersbychentuModItems.ANYINGHEIYAOSHIWUQI.get()));
    }

    @SubscribeEvent
    public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
        event.getSpawns().getSpawner(MobCategory.CREATURE).add(new MobSpawnSettings.SpawnerData(AnnoyingVillagersbychentuModEntities.DIJIYINFENSHEN.get(), 4, 1, 2));
    }

    public static void init() {
        SpawnPlacements.register((EntityType) AnnoyingVillagersbychentuModEntities.DIJIYINFENSHEN.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
            return world.getBlockState(pos.below()).getMaterial() == Material.GRASS && world.getRawBrightness(pos, 0) > 8;
        });
        DungeonHooks.addDungeonMob(AnnoyingVillagersbychentuModEntities.DIJIYINFENSHEN.get(), 180);
    }

    public static AttributeSupplier.Builder createAttributes() {
        AttributeSupplier.Builder builder = Mob.createMobAttributes();
        return builder.add(Attributes.MOVEMENT_SPEED, 0.3d).add(Attributes.MAX_HEALTH, 40.0d).add(Attributes.ARMOR, 20.0d).add(Attributes.ATTACK_DAMAGE, 4.0d).add(Attributes.FOLLOW_RANGE, 128.0d);
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(25, new MeleeAttackGoal(this, 1.5d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.DijiyinfenshenEntity.1
            @Override
    ...
    public boolean hurt(DamageSource source, float amount) {
        DijiyinfenshenDangShiTiShouShangShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
        ...
        DijiyinfenshenShiTiChuShiShengChengShiProcedure.execute(world, this);
        return retval;
    }
    ...
    public void baseTick() {
        super.baseTick();
        DijiyinfenshenDangShiTiGengXinKeShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }

    @Override
    public void playerTouch(Player sourceentity) {
        super.playerTouch(sourceentity);
        DijiyinfenshenDangWanJiaYuGaiShiTiFaShengPengZhuangShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }
}

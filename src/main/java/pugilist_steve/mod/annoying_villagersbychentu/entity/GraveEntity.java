...
public class GraveEntity extends PathfinderMob {
    public GraveEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.GRAVE.get(), world);
    }

    public GraveEntity(EntityType<GraveEntity> type, Level world) {
        super(type, world);
        this.maxUpStep = 4.0f;
        this.xpReward = 50;
        this.setNoAi(false);
    ...
    public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
        event.getSpawns().getSpawner(MobCategory.CREATURE).add(new MobSpawnSettings.SpawnerData(AnnoyingVillagersbychentuModEntities.GRAVE.get(), 1, 1, 1));
    }

    public static void init() {
        SpawnPlacements.register((EntityType) AnnoyingVillagersbychentuModEntities.GRAVE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        ...
        return builder.add(Attributes.MOVEMENT_SPEED, 0.26d).add(Attributes.MAX_HEALTH, 20.0d).add(Attributes.ARMOR, 0.0d).add(Attributes.ATTACK_DAMAGE, 1.0d).add(Attributes.FOLLOW_RANGE, 128.0d);
    }
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(19, new NearestAttackableTargetGoal(this, Player.class, true, true));
        this.goalSelector.addGoal(20, new MeleeAttackGoal(this, 1.5d, false) { // from class: pugilist_steve.mod.annoying_villagersbychentu.entity.GraveEntity.1
            @Override
        ...
        this.goalSelector.addGoal(22, new WaterAvoidingRandomStrollGoal(this, 1.5d));
        this.goalSelector.addGoal(23, new RandomSwimmingGoal(this, 1.5d, 40));
    ...
    public boolean hurt(DamageSource source, float amount) {
        GraveDangShiTiShouShangShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    ...
    public void die(DamageSource source) {
        super.die(source);
        GraveDangShiTiSiWangShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }

    @Override
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
        SpawnGroupData retval = super.finalizeSpawn(world, difficulty, reason, livingdata, tag);
        GraveShiTiChuShiShengChengShiProcedure.execute(world, this.getX(), this.getY(), this.getZ(), this);
        return retval;
    }
    ...
    public InteractionResult mobInteract(Player sourceentity, InteractionHand hand) {
        ...
        GraveDangYouJiShiTiShiProcedure.execute(world, x, y, z, this);
        return retval;
    }
    ...
    public void baseTick() {
        super.baseTick();
        Grave2DangShiTiGengXinKeShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }

    @Override
    public void playerTouch(Player sourceentity) {
        super.playerTouch(sourceentity);
        GraveDangWanJiaYuGaiShiTiFaShengPengZhuangShiProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
    }
}

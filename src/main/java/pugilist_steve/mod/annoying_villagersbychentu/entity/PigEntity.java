...
public class PigEntity extends Pig {
    public PigEntity(PlayMessages.SpawnEntity packet, Level world) {
        this(AnnoyingVillagersbychentuModEntities.PIG.get(), world);
    }
    ...
    public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
        event.getSpawns().getSpawner(MobCategory.CREATURE).add(new MobSpawnSettings.SpawnerData(AnnoyingVillagersbychentuModEntities.PIG.get(), 6, 1, 2));
    }

    public static void init() {
        SpawnPlacements.register((EntityType) AnnoyingVillagersbychentuModEntities.PIG.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        ...
        return builder.add(Attributes.MOVEMENT_SPEED, 0.29d).add(Attributes.MAX_HEALTH, 10.0d).add(Attributes.ARMOR, 40.0d).add(Attributes.ATTACK_DAMAGE, 6.0d).add(Attributes.FOLLOW_RANGE, 128.0d).add(Attributes.ATTACK_KNOCKBACK, 1.0d);
    }
